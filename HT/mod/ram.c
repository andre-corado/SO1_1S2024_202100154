...
static int escribir_archivo(struct seq_file *archivo, void *v)
{
    ...
    unsigned long percentUsed, percentFree;
    
    init_meminfo();

    total = si.totalram * (unsigned long long)si.mem_unit / megabyte;
    free = (si.freeram + si.bufferram) * (unsigned long long)si.mem_unit / megabyte;
    used = total - free;
    percentUsed = (used * 100) / total;
    percentFree = (free * 100) / total;

    seq_printf(archivo, "{\"total\": %lu, \"used\": %lu, \"free\": %lu, \"percentUsed\": %lu, \"percentFree\": %lu}\n", total, used, free, percentUsed, percentFree);
...
{
    proc_create("ram_202100154", 0, NULL, &operaciones);
...
{
    remove_proc_entry("ram_202100154", NULL);
